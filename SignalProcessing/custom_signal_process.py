...
'''
# inspired from radial sdk github
# class used to compute the range azimuth maps or range doppler maps
# will compute the labels needed for the training of the neural network
...
class RadarSignalP7():
    def __init__(self,path_calib_mat,method='PC',device='cpu',lib='CuPy'):
        ...
        if(self.device =='cuda'):
            if(self.lib=='CuPy'):
                raise ImportError('CuPy is not supported in this version. Please use PyTorch instead.')
                ...
                raise NotImplementedError('PyTorch is not supported in this version. Please use CuPy instead.')
    ...
    def get_window_hanning_range(self):
        return self.range_fft_coef
    
    def get_window_hanning_dopller(self):
        return self.doppler_fft_coef

      
    def get_first_fft(self,adc0,adc1,adc2,adc3):
        # 1. Decode the input ADC stream to buld radar complex frames
        complex_adc = self.__build_radar_frame(adc0,adc1,adc2,adc3)
    ...
    def get_torch_first_fft(self,adc0,adc1,adc2,adc3):
        # 1. Decode the input ADC stream to buld radar complex frames
        complex_adc = self.__build_radar_frame(adc0,adc1,adc2,adc3)
        ...
        # 3- Range FFTs
        range_fft = torch.fft.fft(np.multiply(complex_adc,self.range_fft_coef),self.numSamplePerChirp,axis=0)
    ...
    # 
    def get_first_fftV2(self,complex_adc):
        

        # 3- Range FFTs
        signal_windowed=np.multiply(complex_adc,self.range_fft_coef)

        #range_fft = fft(x=signal_windowed,n=self.numSamplePerChirp,axis=0)
        ...
        #dft_matrix = np.exp(-2j * np.pi * j * k / N) / np.sqrt(N)

        dft_matrix = np.exp(-2j * np.pi * j * k / N) 

        # Apply the DFT matrix to the signal
        range_fftv2 = np.tensordot(dft_matrix, signal_windowed, axes=([1], [0]))

        #assert np.allclose(range_fft, range_fftv2)==True, "values differ"

    
        return range_fftv2
    
    def build_fft_by_dot_product(self,complex_adc):
        signal_windowed=np.multiply(complex_adc,self.range_fft_coef)
        N = signal_windowed.shape[0]
    ...
    def compute_second_fft(self,range_fft):
        RD_spectrums = fft(np.multiply(range_fft,self.doppler_fft_coef),self.numChirps,axis=1)
        return RD_spectrums
    
    def compute_second_fft_torch(self,range_fft):
        torch_range_fft=torch.from_numpy(np.multiply(range_fft,self.doppler_fft_coef))
        RD_spectrums = torch.fft.fft(torch_range_fft,self.numChirps,axis=1)
        return RD_spectrums


    def build_fft_matrix(self,complex_adc):
        signal_windowed=np.multiply(complex_adc,self.range_fft_coef)
        N = signal_windowed.shape[0]
    ...
    def __get_RA(self,RD_spectrums):
        ...
        if(self.device=='cpu'):
            ...
            if(self.lib=='CuPy'):
                ...
                raise NotImplementedError('PyTorch is not supported in this version. Please use cpu instead.')
...
# cfar is an algo to detect the presence of a target.
# won't be used in the p7 project
class CA_CFAR():
    """
    Description:
    ------------
